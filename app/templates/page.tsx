      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        {TEMPLATES.map(t => (
          <div key={t.id} style={{ border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden', background: 'white' }}>
            <div style={{ height: 80, background: TYPE_COLORS[t.type]?.bg ?? '#f7f7f5', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: TYPE_COLORS[t.type]?.color ?? '#787774', letterSpacing: '0.5px', textTransform: 'uppercase' }}>{TYPE_LABELS[t.type] ?? t.type}</span>
            </div>
